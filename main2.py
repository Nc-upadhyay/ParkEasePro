...
# Your existing code for parking space picker
width, height = 107, 48
...
def showParkingSpaces(image_path):
    img = cv2.imread(image_path)
    for pos in posList:
        cv2.rectangle(img, pos, (pos[0] + width, pos[1] + height), (255, 0, 255), 2)

    cv2.imshow("Parking Spaces", img)
    cv2.setMouseCallback("Parking Spaces", mouseClick)
    cv2.waitKey(1)


# Your existing code for the welcome page
root = Tk()
root.title("Merged Window")

# Additional Tkinter GUI components
label1 = Label(root, text="Welcome to the Merged Window", font=("Helvetica", 16))
label1.pack()

# Main Loop
while True:
    # Show Parking Spaces on the background image
    showParkingSpaces('./images/carParkImg.png')
    ...
    # Break the loop if the user closes the Tkinter window
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

# Destroy the Tkinter window when the loop ends
root.destroy()
